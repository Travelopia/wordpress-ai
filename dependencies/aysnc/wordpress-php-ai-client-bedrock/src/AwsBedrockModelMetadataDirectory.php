...
use Travelopia\WordPress_AI\Dependencies\WordPress\AiClient\Providers\ApiBasedImplementation\AbstractApiBasedModelMetadataDirectory;
...
class AwsBedrockModelMetadataDirectory extends AbstractApiBasedModelMetadataDirectory
{
    ...
    {
        $request = new Request(HttpMethodEnum::GET(), AwsBedrockProvider::controlPlaneUrl('foundation-models'), ['Content-Type' => 'application/json']);
    ...
    protected function parseResponseToModelMetadataList(Response $response): array
    {
        ...
        foreach ($response_data['modelSummaries'] as $model_data) {
            ...
            $models[$model_id] = new ModelMetadata($model_id, $model_name, $capabilities, $options);
        }
        return $models;
    }
    /**
    ...
    protected function inferCapabilities(array $model_data): array
    {
        ...
        if (in_array('ON_DEMAND', $inference_types, \true)) {
        ...
        if (in_array('IMAGE', $output_modalities, \true)) {
    ...
    protected function getSupportedOptions(array $model_data): array
    {
        ...
        $options = [new SupportedOption(OptionEnum::systemInstruction()), new SupportedOption(OptionEnum::maxTokens()), new SupportedOption(OptionEnum::temperature()), new SupportedOption(OptionEnum::topP()), new SupportedOption(OptionEnum::stopSequences()), new SupportedOption(OptionEnum::customOptions())];
        // Check if model supports multimodal input.
        $input_modalities = $model_data['inputModalities'] ?? [];
        if (in_array('TEXT', $input_modalities, \true) && in_array('IMAGE', $input_modalities, \true)) {
            $options[] = new SupportedOption(OptionEnum::inputModalities(), [[ModalityEnum::text()], [ModalityEnum::text(), ModalityEnum::image()]]);
        } elseif (in_array('TEXT', $input_modalities, \true)) {
            $options[] = new SupportedOption(OptionEnum::inputModalities(), [[ModalityEnum::text()]]);
        }
        // Check output modalities.
        $output_modalities = $model_data['outputModalities'] ?? [];
        if (in_array('TEXT', $output_modalities, \true)) {
            $options[] = new SupportedOption(OptionEnum::outputModalities(), [[ModalityEnum::text()]]);
        }
        // Tool support for models that support it (most Converse API models).
        if (in_array('ON_DEMAND', $model_data['inferenceTypesSupported'] ?? [], \true)) {
