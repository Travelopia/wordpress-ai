...
class AwsBedrockTextGenerationModel extends AbstractApiBasedModel implements TextGenerationModelInterface
{
    ...
    final public function generateTextResult(array $prompt): GenerativeAiResult
    {
        ...
        // Build request.
        $request = new Request(HttpMethodEnum::POST(), AwsBedrockProvider::runtimeUrl("model/{$model_id}/converse", $region), ['Content-Type' => 'application/json'], $params, $this->getRequestOptions());
    ...
    protected function prepareConverseParams(array $prompt): array
    {
        ...
        $params = ['messages' => $this->prepareMessagesParam($prompt)];
        // System instruction.
        $system_instruction = $config->getSystemInstruction();
        if ($system_instruction) {
            $params['system'] = [['text' => $system_instruction]];
        }
        ...
        if (null !== $function_declarations && !empty($function_declarations)) {
            $params['toolConfig'] = ['tools' => $this->prepareFunctionDeclarations($function_declarations)];
        }
        return $params;
    }
    /**
        ...
        foreach ($prompt as $message) {
            ...
            foreach ($message->getParts() as $part) {
                if (null !== $part->getText()) {
                    ...
                    $content[] = ['toolResult' => ['toolUseId' => $function_response->getId(), 'content' => [['json' => $function_response->getResponse()]]]];
                } elseif (null !== $part->getFunctionCall()) {
                    $function_call = $part->getFunctionCall();
                    $content[] = ['toolUse' => ['toolUseId' => $function_call->getId(), 'name' => $function_call->getName(), 'input' => $function_call->getArgs()]];
                }
            }
    ...
    protected function prepareFileContent(File $file): array
    {
        ...
        if (str_starts_with($mime_type, 'image/')) {
            return ['image' => ['format' => $this->getImageFormat($mime_type), 'source' => ['bytes' => $base64_data]]];
        }
        // Handle document files.
        if (str_starts_with($mime_type, 'application/pdf') || str_starts_with($mime_type, 'text/')) {
            return ['document' => ['format' => $this->getDocumentFormat($mime_type), 'name' => 'document', 'source' => ['bytes' => $base64_data]]];
        }
        // phpcs:disable WordPress.Security.EscapeOutput.ExceptionNotEscaped -- Exception message, not rendered output.
        throw new InvalidArgumentException(sprintf('Unsupported file MIME type: %s', $mime_type));
    ...
    {
        $formats = ['application/pdf' => 'pdf', 'text/plain' => 'txt', 'text/html' => 'html', 'text/csv' => 'csv', 'text/markdown' => 'md'];
    ...
    protected function prepareFunctionDeclarations(array $function_declarations): array
    {
        ...
        foreach ($function_declarations as $declaration) {
            $tools[] = ['toolSpec' => ['name' => $declaration->getName(), 'description' => $declaration->getDescription(), 'inputSchema' => ['json' => $declaration->getParameters()]]];
        }
        return $tools;
    }
    /**
    ...
    protected function parseResponseToGenerativeAiResult(Response $response): GenerativeAiResult
    {
        ...
        foreach ($message_data['content'] as $content_item) {
            if (isset($content_item['text'])) {
                ...
                $parts[] = new MessagePart(new FunctionCall($tool_use['toolUseId'], $tool_use['name'], $tool_use['input']));
            }
        }
        ...
        $token_usage = new TokenUsage($usage['inputTokens'] ?? 0, $usage['outputTokens'] ?? 0, $usage['totalTokens'] ?? 0);
        ...
        $id = uniqid('bedrock_', \true);
        return new GenerativeAiResult($id, [$candidate], $token_usage, $this->providerMetadata(), $this->metadata(), $additional_data);
    }
    /**
    ...
    {
        $mapping = ['end_turn' => FinishReasonEnum::stop(), 'max_tokens' => FinishReasonEnum::length(), 'stop_sequence' => FinishReasonEnum::stop(), 'tool_use' => FinishReasonEnum::toolCalls(), 'content_filtered' => FinishReasonEnum::contentFilter()];
