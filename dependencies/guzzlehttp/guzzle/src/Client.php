...
use Travelopia\WordPress_AI\Dependencies\GuzzleHttp\Cookie\CookieJar;
use Travelopia\WordPress_AI\Dependencies\GuzzleHttp\Exception\GuzzleException;
use Travelopia\WordPress_AI\Dependencies\GuzzleHttp\Exception\InvalidArgumentException;
use Travelopia\WordPress_AI\Dependencies\GuzzleHttp\Handler\CurlShareHandleState;
 ...
 * @final
 */
class Client implements ClientInterface, \Travelopia\WordPress_AI\Dependencies\Psr\Http\Client\ClientInterface
{
    use ClientTrait;
    /**
    ...
    public function requestAsync(string $method, $uri = '', array $options = []): PromiseInterface
    {
        ...
        if ($method !== $normalizedMethod) {
            \Travelopia\WordPress_AI\Dependencies\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing a non-uppercase HTTP method to Client::requestAsync() is deprecated; guzzlehttp/guzzle 8.0 will preserve HTTP method casing. Pass an uppercase method explicitly if uppercase is required.');
    ...
    public function request(string $method, $uri = '', array $options = []): ResponseInterface
    {
        $normalizedMethod = \strtoupper($method);
        if ($method !== $normalizedMethod) {
            \Travelopia\WordPress_AI\Dependencies\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing a non-uppercase HTTP method to Client::request() is deprecated; guzzlehttp/guzzle 8.0 will preserve HTTP method casing. Pass an uppercase method explicitly if uppercase is required.');
            $method = $normalizedMethod;
        }
        $options[RequestOptions::SYNCHRONOUS] = \true;
    ...
    {
        \Travelopia\WordPress_AI\Dependencies\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing %s to request option "%s" is deprecated; guzzlehttp/guzzle 8.0 requires %s.', \get_debug_type($value), $option, $expected);
    }
    /**
    ...
    private function transfer(RequestInterface $request, array $options): PromiseInterface
    {
        ...
        if ('' === $protocolVersion) {
            ...
            \Travelopia\WordPress_AI\Dependencies\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Sending a request with a malformed protocol version is deprecated; guzzlehttp/guzzle 8.0 will reject malformed protocol versions.');
        }
    ...
    {
        if ('' === $version) {
            \Travelopia\WordPress_AI\Dependencies\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing an empty "version" request option is deprecated; guzzlehttp/guzzle 8.0 will reject empty protocol versions.');
            return '1.1';
        }
