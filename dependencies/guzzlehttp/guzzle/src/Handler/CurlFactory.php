...
namespace Travelopia\WordPress_AI\Dependencies\GuzzleHttp\Handler;

use Travelopia\WordPress_AI\Dependencies\GuzzleHttp\Exception\ConnectException;
use Travelopia\WordPress_AI\Dependencies\GuzzleHttp\Exception\RequestException;
use Travelopia\WordPress_AI\Dependencies\GuzzleHttp\Promise as P;
use Travelopia\WordPress_AI\Dependencies\GuzzleHttp\Promise\FulfilledPromise;
use Travelopia\WordPress_AI\Dependencies\GuzzleHttp\Promise\PromiseInterface;
use Travelopia\WordPress_AI\Dependencies\GuzzleHttp\Psr7\LazyOpenStream;
use Travelopia\WordPress_AI\Dependencies\GuzzleHttp\TransferStats;
use Travelopia\WordPress_AI\Dependencies\GuzzleHttp\TransportSharing;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    public function create(RequestInterface $request, array $options): EasyHandle
    {
        ...
        if ('' === $protocolVersion) {
            \Travelopia\WordPress_AI\Dependencies\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Sending a request with an empty protocol version is deprecated; guzzlehttp/guzzle 8.0 will reject empty protocol versions.');
            $protocolVersion = '1.1';
            $request = \Travelopia\WordPress_AI\Dependencies\GuzzleHttp\Psr7\Utils::modifyRequest($request, ['version' => $protocolVersion]);
        }
    ...
    private static function triggerConflictingCurlOptionDeprecations(array $options): void
    {
        ...
        foreach ($options['curl'] as $option => $_) {
            ...
            if ($replacement !== null) {
                \Travelopia\WordPress_AI\Dependencies\trigger_deprecation('guzzlehttp/guzzle', '7.11', \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed request handling. Use %s instead.', $name, $replacement));
                continue;
            }
            \Travelopia\WordPress_AI\Dependencies\trigger_deprecation('guzzlehttp/guzzle', '7.11', \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed cURL internals.', $name));
        }
    }
    private static function triggerUnsupportedRequestOptionDeprecations(array $options): void
    {
        if (\array_key_exists('transport_sharing', $options) && CurlShareHandleState::normalizeMode($options['transport_sharing'], 'transport_sharing') !== TransportSharing::NONE) {
            \Travelopia\WordPress_AI\Dependencies\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "transport_sharing" request option to a cURL handler is deprecated; guzzlehttp/guzzle 8.0 will reject this option because transport sharing must be configured when creating the Client, CurlHandler, or CurlMultiHandler.');
        }
        if (\array_key_exists('stream_context', $options)) {
            \Travelopia\WordPress_AI\Dependencies\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "stream_context" request option to a cURL handler is deprecated; guzzlehttp/guzzle 8.0 will reject this option because cURL handlers ignore PHP stream context options.');
        }
    }
    /**
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \Travelopia\WordPress_AI\Dependencies\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \Travelopia\WordPress_AI\Dependencies\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \Travelopia\WordPress_AI\Dependencies\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \Travelopia\WordPress_AI\Dependencies\GuzzleHttp\Psr7\Utils::streamFor($sink);
