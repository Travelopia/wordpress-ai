...
use Travelopia\WordPress_AI\Dependencies\GuzzleHttp\Handler\CurlHandler;
use Travelopia\WordPress_AI\Dependencies\GuzzleHttp\Handler\CurlMultiHandler;
use Travelopia\WordPress_AI\Dependencies\GuzzleHttp\Handler\CurlShareHandleState;
use Travelopia\WordPress_AI\Dependencies\GuzzleHttp\Handler\Proxy;
use Travelopia\WordPress_AI\Dependencies\GuzzleHttp\Handler\StreamHandler;
...
EOT
    ...
    public static function normalizeIdnConversionOption($value): ?int
    {
        ...
        if (\is_string($value) && \is_numeric($value) || \is_float($value) && \is_finite($value)) {
            \Travelopia\WordPress_AI\Dependencies\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing %s as the "idn_conversion" request option is deprecated; guzzlehttp/guzzle 8.0 will reject values that are not true, false, null, or an integer IDNA_* bitmask.', self::describeType($value));
            return (int) $value;
        }
