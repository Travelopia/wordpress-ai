...
namespace Travelopia\WordPress_AI\Dependencies\WordPress\AiClient\Messages\DTO;
...
class Message extends AbstractDataTransferObject
{
    ...
    {
        return ['type' => 'object', 'properties' => [self::KEY_ROLE => ['type' => 'string', 'enum' => MessageRoleEnum::getValues(), 'description' => 'The role of the message sender.'], self::KEY_PARTS => ['type' => 'array', 'items' => MessagePart::getJsonSchema(), 'minItems' => 1, 'description' => 'The parts that make up this message.']], 'required' => [self::KEY_ROLE, self::KEY_PARTS]];
    }
    /**
     * {@inheritDoc}
     *
    ...
    {
        return [self::KEY_ROLE => $this->role->value, self::KEY_PARTS => array_map(function (MessagePart $part) {
            return $part->toArray();
        }, $this->parts)];
    }
    /**
