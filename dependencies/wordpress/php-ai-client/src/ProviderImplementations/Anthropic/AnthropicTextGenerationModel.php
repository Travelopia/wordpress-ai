...
class AnthropicTextGenerationModel extends AbstractApiBasedModel implements TextGenerationModelInterface
{
    ...
    final public function generateTextResult(array $prompt): GenerativeAiResult
    {
        ...
        $request = new Request(HttpMethodEnum::POST(), AnthropicProvider::url('messages'), $headers, $params, $this->getRequestOptions());
    ...
    {
        $config = $this->getConfig();
        $params = ['model' => $this->metadata()->getId(), 'messages' => $this->prepareMessagesParam($prompt)];
        ...
        if ($outputMimeType === 'application/json' && $outputSchema) {
            $params['output_format'] = ['type' => 'json_schema', 'schema' => $outputSchema];
        }
    ...
    protected function prepareMessagesParam(array $messages): array
    {
        return array_map(function (Message $message): array {
            return ['role' => $this->getMessageRoleString($message->getRole()), 'content' => array_values(array_filter(array_map([$this, 'getMessagePartData'], $message->getParts())))];
        }, $messages);
    }
    /**
    ...
    protected function getMessagePartData(MessagePart $part): ?array
    {
        ...
        if ($type->isText()) {
            if ($part->getChannel()->isThought()) {
                return ['type' => 'thinking', 'thinking' => $part->getText()];
            }
        ...
        if ($type->isFile()) {
            ...
            if ($file->isRemote()) {
                ...
                throw new InvalidArgumentException('Unsupported file type: The API only supports inline files for non-document types.');
            }
            ...
            if ($file->isImage()) {
                return ['type' => 'image', 'source' => array('type' => 'base64', 'media_type' => $file->getMimeType(), 'data' => $fileBase64Data)];
            }
            if ($file->isDocument()) {
                return ['type' => 'document', 'source' => ['type' => 'base64', 'media_type' => $file->getMimeType(), 'data' => $fileBase64Data]];
            }
        ...
        if ($type->isFunctionCall()) {
            ...
            return ['type' => 'tool_use', 'id' => $functionCall->getId(), 'name' => $functionCall->getName(), 'input' => $input];
        }
        if ($type->isFunctionResponse()) {
            ...
            return ['type' => 'tool_result', 'tool_use_id' => $functionResponse->getId(), 'content' => json_encode($functionResponse->getResponse())];
        }
    ...
    protected function prepareToolsParam(?array $functionDeclarations, ?WebSearch $webSearch): array
    {
        ...
        if (is_array($functionDeclarations)) {
            foreach ($functionDeclarations as $functionDeclaration) {
                ...
                $tools[] = array_filter(['name' => $functionDeclaration->getName(), 'description' => $functionDeclaration->getDescription(), 'input_schema' => $inputSchema]);
            }
        }
        if ($webSearch) {
            $tools[] = array_filter(['type' => 'web_search_20250305', 'name' => 'web_search', 'max_uses' => 1, 'allowed_domains' => $webSearch->getAllowedDomains(), 'blocked_domains' => $webSearch->getDisallowedDomains()]);
        }
        return $tools;
    }
    /**
    ...
    protected function parseResponseToGenerativeAiResult(Response $response): GenerativeAiResult
    {
        ...
        if (!is_array($responseData['content']) || !array_is_list($responseData['content'])) {
            throw ResponseException::fromInvalidData($this->providerMetadata()->getName(), 'content', 'The value must be an indexed array.');
        }
        $role = isset($responseData['role']) && 'user' === $responseData['role'] ? MessageRoleEnum::user() : MessageRoleEnum::model();
        $parts = [];
        foreach ($responseData['content'] as $partIndex => $messagePartData) {
            try {
                ...
                throw ResponseException::fromInvalidData($this->providerMetadata()->getName(), "content[{$partIndex}]", $e->getMessage());
            }
        }
        ...
        switch ($responseData['stop_reason']) {
            ...
            case 'tool_use':
                ...
                break;
            default:
                throw ResponseException::fromInvalidData($this->providerMetadata()->getName(), 'stop_reason', sprintf('Invalid stop reason "%s".', $responseData['stop_reason']));
        }
        ...
        if (isset($responseData['usage']) && is_array($responseData['usage'])) {
            ...
            $tokenUsage = new TokenUsage($inputTokens, $usage['output_tokens'] ?? 0, $inputTokens + ($usage['output_tokens'] ?? 0));
        } else {
    ...
    protected function parseResponseContentMessagePart(array $partData): ?MessagePart
    {
        ...
        switch ($partData['type']) {
            ...
            case 'tool_use':
                if (!isset($partData['id']) || !is_string($partData['id']) || !isset($partData['name']) || !is_string($partData['name']) || !isset($partData['input'])) {
