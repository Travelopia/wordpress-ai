...
use Travelopia\WordPress_AI\Dependencies\WordPress\AiClient\Providers\Models\ImageGeneration\Contracts\ImageGenerationModelInterface;
...
class GoogleImageGenerationModel extends AbstractApiBasedModel implements ImageGenerationModelInterface
{
    ...
    public function generateImageResult(array $prompt): GenerativeAiResult
    {
        ...
        $request = new Request(HttpMethodEnum::POST(), GoogleProvider::url("models/{$this->metadata()->getId()}:predict"), ['Content-Type' => 'application/json'], $params, $this->getRequestOptions());
        // Add authentication credentials to the request.
        $request = $this->getRequestAuthentication()->authenticateRequest($request);
        // Send and process the request.
        $response = $httpTransporter->send($request);
        ResponseUtil::throwIfNotSuccessful($response);
        return $this->parseResponseToGenerativeAiResult($response, $params['parameters']['outputOptions']['mimeType'] ?? 'image/png');
    }
    /**
    ...
    protected function prepareGenerateImageParams(array $prompt): array
    {
        ...
        $params = ['instances' => [['prompt' => $this->preparePromptParam($prompt)]], 'parameters' => ['sampleCount' => 1]];
        ...
        if ($config->getOutputFileType() && $config->getOutputFileType()->isRemote()) {
            throw new InvalidArgumentException('Unsupported output file type: Only inline is supported.');
        }
        ...
        foreach ($customOptions as $key => $value) {
            ...
            if (str_starts_with($key, 'parameters.')) {
                ...
                if (isset($params['parameters'][$key])) {
                    throw new InvalidArgumentException(sprintf('The custom parameters option "%s" conflicts with an existing parameter.', $key));
                }
                $params['parameters'][$key] = $value;
                continue;
            }
    ...
    protected function parseResponseToGenerativeAiResult(Response $response, string $expectedMimeType = 'image/png'): GenerativeAiResult
    {
        ...
        if (!is_array($responseData['predictions'])) {
            throw ResponseException::fromInvalidData($this->providerMetadata()->getName(), 'predictions', 'The value must be an array.');
        }
        $candidates = [];
        foreach ($responseData['predictions'] as $index => $predictionData) {
            if (!is_array($predictionData) || array_is_list($predictionData)) {
                throw ResponseException::fromInvalidData($this->providerMetadata()->getName(), "predictions[{$index}]", 'The value must be an associative array.');
            }
        ...
        return new GenerativeAiResult($id, $candidates, new TokenUsage(0, 0, 0), $this->providerMetadata(), $this->metadata(), $providerMetadata);
    }
    /**
     ...
     */
    protected function parseResponsePredictionToCandidate(array $predictionData, int $index, string $expectedMimeType = 'image/png'): Candidate
    {
        ...
        if (isset($predictionData['url']) && is_string($predictionData['url'])) {
            ...
            throw ResponseException::fromInvalidData($this->providerMetadata()->getName(), "predictions[{$index}]", 'The value must contain either a url or bytesBase64Encoded key with a string value.');
        }
