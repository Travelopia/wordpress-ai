...
namespace Travelopia\WordPress_AI\Dependencies\WordPress\AiClient\ProviderImplementations\Google;

use Travelopia\WordPress_AI\Dependencies\WordPress\AiClient\Common\Exception\InvalidArgumentException;
use Travelopia\WordPress_AI\Dependencies\WordPress\AiClient\Common\Exception\RuntimeException;
use Travelopia\WordPress_AI\Dependencies\WordPress\AiClient\Files\DTO\File;
use Travelopia\WordPress_AI\Dependencies\WordPress\AiClient\Messages\DTO\Message;
use Travelopia\WordPress_AI\Dependencies\WordPress\AiClient\Messages\DTO\MessagePart;
use Travelopia\WordPress_AI\Dependencies\WordPress\AiClient\Messages\Enums\MessagePartChannelEnum;
use Travelopia\WordPress_AI\Dependencies\WordPress\AiClient\Messages\Enums\MessageRoleEnum;
use Travelopia\WordPress_AI\Dependencies\WordPress\AiClient\Messages\Enums\ModalityEnum;
use Travelopia\WordPress_AI\Dependencies\WordPress\AiClient\Providers\ApiBasedImplementation\AbstractApiBasedModel;
use Travelopia\WordPress_AI\Dependencies\WordPress\AiClient\Providers\Http\Contracts\RequestAuthenticationInterface;
...
use Travelopia\WordPress_AI\Dependencies\WordPress\AiClient\Providers\Http\Util\ResponseUtil;
use Travelopia\WordPress_AI\Dependencies\WordPress\AiClient\Providers\Models\TextGeneration\Contracts\TextGenerationModelInterface;
use Travelopia\WordPress_AI\Dependencies\WordPress\AiClient\Results\DTO\Candidate;
use Travelopia\WordPress_AI\Dependencies\WordPress\AiClient\Results\DTO\GenerativeAiResult;
use Travelopia\WordPress_AI\Dependencies\WordPress\AiClient\Results\DTO\TokenUsage;
use Travelopia\WordPress_AI\Dependencies\WordPress\AiClient\Results\Enums\FinishReasonEnum;
use Travelopia\WordPress_AI\Dependencies\WordPress\AiClient\Tools\DTO\FunctionCall;
use Travelopia\WordPress_AI\Dependencies\WordPress\AiClient\Tools\DTO\FunctionDeclaration;
/**
...
class GoogleTextGenerationModel extends AbstractApiBasedModel implements TextGenerationModelInterface
{
    ...
    final public function generateTextResult(array $prompt): GenerativeAiResult
    {
        ...
        $request = new Request(HttpMethodEnum::POST(), GoogleProvider::url("models/{$this->metadata()->getId()}:generateContent"), ['Content-Type' => 'application/json'], $params, $this->getRequestOptions());
    ...
    protected function prepareGenerateTextParams(array $prompt): array
    {
        ...
        $params = ['contents' => $this->prepareContentsParam($prompt)];
        ...
        if (is_array($functionDeclarations)) {
            $tools[] = ['functionDeclarations' => $this->prepareFunctionDeclarationsParam($functionDeclarations)];
        }
        ...
        foreach ($customOptions as $key => $value) {
            ...
            if (str_starts_with($key, 'generationConfig.')) {
                ...
                if (isset($params['generationConfig'][$key])) {
                    throw new InvalidArgumentException(sprintf('The custom generationConfig option "%s" conflicts with an existing parameter.', $key));
                }
                $params['generationConfig'][$key] = $value;
                continue;
            }
    ...
    protected function prepareContentsParam(array $messages): array
    {
        return array_map(function (Message $message): array {
            return ['role' => $this->getMessageRoleString($message->getRole()), 'parts' => array_values(array_filter(array_map([$this, 'getMessagePartData'], $message->getParts())))];
        }, $messages);
    }
    /**
    ...
    protected function getMessagePartData(MessagePart $part): ?array
    {
        ...
        if ($type->isText()) {
            if ($part->getChannel()->isThought()) {
                return ['text' => $part->getText(), 'thought' => \true];
            }
        ...
        if ($type->isFile()) {
            ...
            if ($file->isRemote()) {
                ...
                return ['fileData' => ['mimeType' => $file->getMimeType(), 'fileUri' => $fileUrl]];
            }
            ...
            return ['inlineData' => ['mimeType' => $file->getMimeType(), 'data' => $fileBase64Data]];
        }
        ...
        if ($type->isFunctionResponse()) {
            ...
            return ['functionResponse' => [
                 ...
                 */
                'response' => ['name' => $functionResponse->getName(), 'content' => $functionResponse->getResponse()],
            ]];
        }
    ...
    protected function parseResponseToGenerativeAiResult(Response $response): GenerativeAiResult
    {
        ...
        if (!is_array($responseData['candidates'])) {
            throw ResponseException::fromInvalidData($this->providerMetadata()->getName(), 'candidates', 'The value must be an array.');
        }
        $candidates = [];
        foreach ($responseData['candidates'] as $index => $candidateData) {
            if (!is_array($candidateData) || array_is_list($candidateData)) {
                throw ResponseException::fromInvalidData($this->providerMetadata()->getName(), "candidates[{$index}]", 'The value must be an associative array.');
            }
        ...
        if (isset($responseData['usageMetadata']) && is_array($responseData['usageMetadata'])) {
            ...
            $tokenUsage = new TokenUsage($usage['promptTokenCount'] ?? 0, $usage['candidatesTokenCount'] ?? 0, ($usage['candidatesTokenCount'] ?? 0) + ($usage['thoughtsTokenCount'] ?? 0));
        } else {
    ...
    {
        if (!isset($candidateData['content']) || !is_array($candidateData['content']) || array_is_list($candidateData['content'])) {
            throw ResponseException::fromMissingData($this->providerMetadata()->getName(), "candidates[{$index}].content");
        }
        if (!isset($candidateData['finishReason']) || !is_string($candidateData['finishReason'])) {
            throw ResponseException::fromMissingData($this->providerMetadata()->getName(), "candidates[{$index}].finishReason");
        }
        ...
        switch ($candidateData['finishReason']) {
            ...
            case 'SPII':
                ...
                break;
            default:
                throw ResponseException::fromInvalidData($this->providerMetadata()->getName(), "candidates[{$index}].finishReason", sprintf('Invalid finish reason "%s".', $candidateData['finishReason']));
        }
    ...
    protected function parseResponseCandidateMessage(array $messageData, int $index): Message
    {
        ...
        if (!isset($messageData['parts'])) {
            throw ResponseException::fromMissingData($this->providerMetadata()->getName(), "candidates[{$index}].content.parts");
        }
        if (!is_array($messageData['parts']) || !array_is_list($messageData['parts'])) {
            throw ResponseException::fromInvalidData($this->providerMetadata()->getName(), "candidates[{$index}].content.parts", 'The value must be an indexed array.');
        }
        $parts = [];
        foreach ($messageData['parts'] as $partIndex => $messagePartData) {
            try {
                ...
                throw ResponseException::fromInvalidData($this->providerMetadata()->getName(), "candidates[{$index}].content.parts[{$partIndex}]", $e->getMessage());
            }
        }
    ...
    protected function parseResponseCandidateMessagePart(array $partData): MessagePart
    {
        ...
        if (isset($partData['inlineData'])) {
            if (!is_array($partData['inlineData']) || !isset($partData['inlineData']['data']) || !is_string($partData['inlineData']['data'])) {
                throw new InvalidArgumentException('Part has an invalid inlineData shape.');
            }
            return new MessagePart(new File($partData['inlineData']['data'], isset($partData['inlineData']['mimeType']) && is_string($partData['inlineData']['mimeType']) ? $partData['inlineData']['mimeType'] : null));
        }
        if (isset($partData['fileData'])) {
            if (!is_array($partData['fileData']) || !isset($partData['fileData']['fileUri']) || !is_string($partData['fileData']['fileUri'])) {
                throw new InvalidArgumentException('Part has an invalid fileData shape.');
            }
            return new MessagePart(new File($partData['fileData']['fileUri'], isset($partData['fileData']['mimeType']) && is_string($partData['fileData']['mimeType']) ? $partData['fileData']['mimeType'] : null));
        }
        if (isset($partData['functionCall'])) {
            if (!is_array($partData['functionCall']) || !isset($partData['functionCall']['name']) || !is_string($partData['functionCall']['name'])) {
            ...
            return new MessagePart(new FunctionCall(null, $partData['functionCall']['name'], $args));
        }
