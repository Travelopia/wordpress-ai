...
use Travelopia\WordPress_AI\Dependencies\WordPress\AiClient\Tools\DTO\WebSearch;
/**
...
class OpenAiTextGenerationModel extends AbstractApiBasedModel implements TextGenerationModelInterface
{
    ...
    final public function generateTextResult(array $prompt): GenerativeAiResult
    {
        ...
        $request = new Request(HttpMethodEnum::POST(), OpenAiProvider::url('responses'), ['Content-Type' => 'application/json'], $params, $this->getRequestOptions());
    ...
    protected function prepareGenerateTextParams(array $prompt): array
    {
        ...
        $params = ['model' => $this->metadata()->getId(), 'input' => $this->prepareInputParam($prompt)];
        ...
        if ($outputMimeType === 'application/json' && $outputSchema) {
            $params['text'] = ['format' => ['type' => 'json_schema', 'name' => 'response_schema', 'schema' => $outputSchema, 'strict' => \true]];
        }
        ...
        foreach ($messages as $message) {
            ...
            foreach ($parts as $part) {
                ...
                if ($type->isFunctionCall()) {
                    throw new InvalidArgumentException('Function call parts must be the only part in a message for the OpenAI Responses API.');
                }
                if ($type->isFunctionResponse()) {
                    throw new InvalidArgumentException('Function response parts must be the only part in a message for the OpenAI Responses API.');
                }
            }
        }
    }
    ...
    protected function getMessageInputItem(Message $message): ?array
    {
        ...
        return ['role' => $this->getMessageRoleString($role), 'content' => $content];
    }
    /**
    ...
    protected function getMessagePartData(MessagePart $part, MessageRoleEnum $role): array
    {
        $type = $part->getType();
        if ($type->isText()) {
            return ['type' => $role->isModel() ? 'output_text' : 'input_text', 'text' => $part->getText()];
        }
        if ($type->isFile()) {
            $file = $part->getFile();
            if (!$file) {
            ...
            return ['type' => 'input_file', 'filename' => 'file', 'file_data' => $dataUri];
        }
        if ($type->isFunctionCall()) {
            ...
            return ['type' => 'function_call', 'call_id' => $functionCall->getId(), 'name' => $functionCall->getName(), 'arguments' => json_encode($functionCall->getArgs())];
        }
        if ($type->isFunctionResponse()) {
            ...
            return ['type' => 'function_call_output', 'call_id' => $functionResponse->getId(), 'output' => json_encode($functionResponse->getResponse())];
        }
    ...
    protected function prepareToolsParam(?array $functionDeclarations, ?WebSearch $webSearch): array
    {
        ...
        if (is_array($functionDeclarations)) {
            foreach ($functionDeclarations as $functionDeclaration) {
                $tools[] = ['type' => 'function', 'name' => $functionDeclaration->getName(), 'description' => $functionDeclaration->getDescription(), 'parameters' => $functionDeclaration->getParameters()];
            }
        }
        if ($webSearch) {
    ...
    protected function parseResponseToGenerativeAiResult(Response $response): GenerativeAiResult
    {
        ...
        if (!is_array($responseData['output']) || !array_is_list($responseData['output'])) {
            throw ResponseException::fromInvalidData($this->providerMetadata()->getName(), 'output', 'The value must be an indexed array.');
        }
        $candidates = [];
        foreach ($responseData['output'] as $index => $outputItem) {
            if (!is_array($outputItem) || array_is_list($outputItem)) {
                throw ResponseException::fromInvalidData($this->providerMetadata()->getName(), "output[{$index}]", 'The value must be an associative array.');
            }
        ...
        if (isset($responseData['usage']) && is_array($responseData['usage'])) {
            ...
            $tokenUsage = new TokenUsage($usage['input_tokens'] ?? 0, $usage['output_tokens'] ?? 0, $usage['total_tokens'] ?? ($usage['input_tokens'] ?? 0) + ($usage['output_tokens'] ?? 0));
        } else {
     ...
     */
    protected function parseMessageOutputToCandidate(array $outputItem, int $index, string $responseStatus): Candidate
    {
        $role = isset($outputItem['role']) && $outputItem['role'] === 'user' ? MessageRoleEnum::user() : MessageRoleEnum::model();
        $parts = [];
                ...
                try {
                    ...
                    if ($part !== null) {
                        ...
                        if ($part->getType()->isFunctionCall()) {
                            $hasFunctionCalls = \true;
                        }
                    }
                } catch (InvalidArgumentException $e) {
                    throw ResponseException::fromInvalidData($this->providerMetadata()->getName(), "output[{$index}].content[{$contentIndex}]", $e->getMessage());
                }
            }
        }
    ...
    protected function parseFunctionCallOutputToCandidate(array $outputItem, int $index): Candidate
    {
        if (!isset($outputItem['call_id']) || !is_string($outputItem['call_id'])) {
            throw ResponseException::fromMissingData($this->providerMetadata()->getName(), "output[{$index}].call_id");
        }
        if (!isset($outputItem['name']) || !is_string($outputItem['name'])) {
            throw ResponseException::fromMissingData($this->providerMetadata()->getName(), "output[{$index}].name");
        }
        /*
        ...
        if (isset($outputItem['arguments']) && is_string($outputItem['arguments'])) {
            $decoded = json_decode($outputItem['arguments'], \true);
            if (is_array($decoded) && count($decoded) > 0) {
                $args = $decoded;
            }
        }
        $functionCall = new FunctionCall($outputItem['call_id'], $outputItem['name'], $args);
    ...
    protected function parseOutputContentToPart(array $contentItem): ?MessagePart
    {
        ...
        if ($type === 'function_call') {
            if (!isset($contentItem['call_id']) || !is_string($contentItem['call_id']) || !isset($contentItem['name']) || !is_string($contentItem['name'])) {
            ...
            if (isset($contentItem['arguments']) && is_string($contentItem['arguments'])) {
                $decoded = json_decode($contentItem['arguments'], \true);
                if (is_array($decoded) && count($decoded) > 0) {
                    $args = $decoded;
                }
            }
            return new MessagePart(new FunctionCall($contentItem['call_id'], $contentItem['name'], $args));
        }
