...
namespace Travelopia\WordPress_AI\Dependencies\WordPress\AiClient\Providers\Models\DTO;

use InvalidArgumentException;
use Travelopia\WordPress_AI\Dependencies\WordPress\AiClient\Common\AbstractDataTransferObject;
...
class ModelRequirements extends AbstractDataTransferObject
{
    ...
    public static function fromPromptData(CapabilityEnum $capability, array $messages, ModelConfig $modelConfig): self
    {
        ...
        if ($hasFunctionMessageParts) {
            $requiredOptions = self::includeInRequiredOptions($requiredOptions, new RequiredOption(OptionEnum::functionDeclarations(), \true));
        }
        ...
        if (!empty($inputModalities)) {
            ...
            $requiredOptions = self::includeInRequiredOptions($requiredOptions, new RequiredOption(OptionEnum::inputModalities(), array_values($inputModalities)));
        }
    ...
    private static function toRequiredOptions(ModelConfig $modelConfig): array
    {
        ...
        if ($modelConfig->getOutputModalities() !== null) {
            $requiredOptions[] = new RequiredOption(OptionEnum::outputModalities(), $modelConfig->getOutputModalities());
        }
        if ($modelConfig->getSystemInstruction() !== null) {
            $requiredOptions[] = new RequiredOption(OptionEnum::systemInstruction(), $modelConfig->getSystemInstruction());
        }
        if ($modelConfig->getCandidateCount() !== null) {
            $requiredOptions[] = new RequiredOption(OptionEnum::candidateCount(), $modelConfig->getCandidateCount());
        }
        if ($modelConfig->getMaxTokens() !== null) {
            $requiredOptions[] = new RequiredOption(OptionEnum::maxTokens(), $modelConfig->getMaxTokens());
        }
        if ($modelConfig->getTemperature() !== null) {
            $requiredOptions[] = new RequiredOption(OptionEnum::temperature(), $modelConfig->getTemperature());
        }
        if ($modelConfig->getTopP() !== null) {
            $requiredOptions[] = new RequiredOption(OptionEnum::topP(), $modelConfig->getTopP());
        }
        if ($modelConfig->getTopK() !== null) {
            $requiredOptions[] = new RequiredOption(OptionEnum::topK(), $modelConfig->getTopK());
        }
        if ($modelConfig->getOutputMimeType() !== null) {
            $requiredOptions[] = new RequiredOption(OptionEnum::outputMimeType(), $modelConfig->getOutputMimeType());
        }
        if ($modelConfig->getOutputSchema() !== null) {
            $requiredOptions[] = new RequiredOption(OptionEnum::outputSchema(), $modelConfig->getOutputSchema());
        }
        ...
        if ($modelConfig->getFrequencyPenalty() !== null) {
            $requiredOptions[] = new RequiredOption(OptionEnum::frequencyPenalty(), $modelConfig->getFrequencyPenalty());
        }
        ...
        if ($modelConfig->getFunctionDeclarations() !== null) {
            $requiredOptions[] = new RequiredOption(OptionEnum::functionDeclarations(), \true);
        }
        if ($modelConfig->getWebSearch() !== null) {
            $requiredOptions[] = new RequiredOption(OptionEnum::webSearch(), \true);
        }
        ...
        if ($modelConfig->getOutputMediaOrientation() !== null) {
            $requiredOptions[] = new RequiredOption(OptionEnum::outputMediaOrientation(), $modelConfig->getOutputMediaOrientation());
        }
        if ($modelConfig->getOutputMediaAspectRatio() !== null) {
            $requiredOptions[] = new RequiredOption(OptionEnum::outputMediaAspectRatio(), $modelConfig->getOutputMediaAspectRatio());
        }
    ...
    {
        return ['type' => 'object', 'properties' => [self::KEY_REQUIRED_CAPABILITIES => ['type' => 'array', 'items' => ['type' => 'string', 'enum' => CapabilityEnum::getValues()], 'description' => 'The capabilities that the model must support.'], self::KEY_REQUIRED_OPTIONS => ['type' => 'array', 'items' => RequiredOption::getJsonSchema(), 'description' => 'The options that the model must support with specific values.']], 'required' => [self::KEY_REQUIRED_CAPABILITIES, self::KEY_REQUIRED_OPTIONS]];
    }
    /**
     * {@inheritDoc}
     *
    ...
    {
        return [self::KEY_REQUIRED_CAPABILITIES => array_map(static fn(CapabilityEnum $capability): string => $capability->value, $this->requiredCapabilities), self::KEY_REQUIRED_OPTIONS => array_map(static fn(RequiredOption $option): array => $option->toArray(), $this->requiredOptions)];
    }
    /**
     * {@inheritDoc}
     *
    ...
    public static function fromArray(array $array): self
    {
        ...
        return new self(array_map(static fn(string $capability): CapabilityEnum => CapabilityEnum::from($capability), $array[self::KEY_REQUIRED_CAPABILITIES]), array_map(static fn(array $optionData): RequiredOption => RequiredOption::fromArray($optionData), $array[self::KEY_REQUIRED_OPTIONS]));
    }
}
